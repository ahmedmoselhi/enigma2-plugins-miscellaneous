...
class HddMountDevice(Screen):
    ...
    def ok(self):
        ...
        if selected == 0:
            ...
            self.session.openWithCallback(
                self.customPath, VirtualKeyBoard, title=(
                    _("Insert mount point:")), text=_("/media/custom"))
    ...
    def setMountPoint(self, path):
        ...
        if self.mountpoints.exist(path):
            self.session.openWithCallback(self.setMountPointCb,
                                          ExtraMessageBox,
                                          _("Selected mount point is already used by another drive."),
                                          _("Mount point exist!"),
                                          [[_("Change old drive with this new drive"),
                                            "ok.png"],
                                              [_("Keep old drive"),
                                               "cancel.png"],
                                           ])
    ...
    def setMountPointCb(self, result):
        if result == 0:
            ...
            if not self.mountpoints.mount(
                    self.device, self.partition, self.cpath):
                self.session.open(
                    MessageBox,
            ...
            if not self.fast:
                message = _(
                    "Device Fixed Mount Point change needs a system restart in order to take effect.\nRestart your STB now?")
            ...
            if (disk[2] or "mmcblk" in disk[0]) and not disk[7]:
                ...
                for partition in disk[5]:
                    ...
                    if len(mp) > 0:
                        self.disks.append(
                            MountEntry(
                                fullname, _("P.%s (Fixed: %s)") %
                                (count, mp)))
                        self.mounts.append(mp)
                    elif len(rmp) > 0:
                        self.disks.append(
                            MountEntry(
                                fullname, _("P.%s (Fast: %s)") %
                                (count, rmp)))
