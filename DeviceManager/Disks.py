...
BOX_MODEL = ""
BOX_NAME = ""
if fileExists("/proc/stb/info/vumodel") and not fileExists("/proc/stb/info/hwmodel") and not fileExists(
        "/proc/stb/info/boxtype") and not fileExists("/proc/stb/info/gbmodel"):
    try:
    ...
    def readDisks(self):
        ...
        for part in partitions:
            ...
            if res and len(res) == 4:
                if (len(res[3]) == 3 and (res[3][:2] == "sd" or res[3][:3] == "hdb")) or (
                        len(res[3]) == 7 and res[3][:7] not in BLACKLIST):
    ...
    def isMounted(self, device):
        ...
        for mount in mounts:
            ...
            if res and len(res) > 1:
                if "mmcblk" in device:
                    if (res[0] == '/dev/%sp1' %
                        device) or (res[0] == '/dev/%sp2' %
                                    device) or (res[0] == '/dev/%sp3' %
                                                device) or (res[0] == '/dev/%sp4' %
                                                            device):
                        mounts.close()
                        return True
