...
class LamedbReader():
    ...
    def parseLamedbV4Content(self, content, transponders):
        ...
        if tp_start < 0 or tp_stop < 0:
            print(
                "[%s-LamedbReader] parseLamedbV4Content: no transponders found" %
                (debug_name))
            return transponders
        ...
        for block in tp_blocks:
            ...
            if transponder["dvb_type"] == "dvbs" and len(
                    second_row) not in (7, 11, 14, 16):
                continue
        ...
        if srv_start < 0 or srv_stop < 0:
            print(
                "[%s-LamedbReader] parseLamedbV4Content: no services found" %
                (debug_name))
            return transponders
        ...
        for i in range(0, len(srv_blocks) // 3):
            ...
            if len(service_reference) == 7 and int(
                    service_reference[6], 16) != 0:
                service["ATSC_source_id"] = int(service_reference[6], 16)

            key = "%x:%x:%x" % (service["namespace"],
            ...
            # Dirty hack to work around the (well known) service type bug in
            # lamedb/enigma2
            transponders[key]["services"]["%x:%x" %
                                          (service["service_id"], service["service_type"])] = service
    ...
    def parseLamedbV5Content(self, content, transponders):
        ...
        for line in lines:
            if line.startswith("t:"):
                ...
                if transponder["dvb_type"] == "dvbs" and len(
                        second_part) not in (7, 11):
                    continue
                if transponder["dvb_type"] == "dvbt" and len(
                        second_part) != 12:
                    continue
                ...
                if transponder["dvb_type"] == "dvbs":
                    ...
                    if len(second_part) == 7:  # DVB-S
                        ...
                        ).split(",")[2:]:  # Multistream/T2MI
                            if part.startswith(
                                    "MIS/PLS:") and len(part[8:].split(":")) == 3:
                                transponder["is_id"] = int(
                                    part[8:].split(":")[0])
                                transponder["pls_code"] = int(
                                    part[8:].split(":")[1])
                                transponder["pls_mode"] = int(
                                    part[8:].split(":")[2])
                            elif part.startswith("T2MI:") and len(part[5:].split(":")) == 2:
                                transponder["t2mi_plp_id"] = int(
                                    part[5:].split(":")[0])
                                transponder["t2mi_pid"] = int(
                                    part[5:].split(":")[1])
                elif transponder["dvb_type"] == "dvbt":
                    transponder["frequency"] = int(second_part[0])
                ...
                # Dirty hack to work around the (well known) service type bug
                # in lamedb/enigma2
                transponders[key]["services"]["%x:%x" % (
                    service["service_id"], service["service_type"])] = service
...
class LamedbWriter():
    def writeLamedb(self, path, transponders, filename="lamedb"):
        ...
        for key in sorted(transponders.keys(), key=lambda x: tuple(
                [int(n, 16) for n in x.split(":")])):
            transponder = transponders[key]
            if "services" not in transponder.keys() or len(
                    transponder["services"]) < 1:
                continue
                ...
                if transponder["system"] == 0:  # DVB-S
                    ...
                    if t2mi and not multistream:  # this is to pad t2mi values if necessary.
                        try:  # some images are still not multistream aware after all this time
                            ...
                            # print("[%s-BouquetsWriter] some images are still not multistream aware after all this time" % (debug_name),  err)
                            pass
                    lamedblist.append(
        ...
        for key in sorted(transponders.keys(), key=lambda x: tuple(
            ...
            for key2 in sorted(
                ...
                lamedblist.append(
                    ...
                    (service["service_id"],
                     service["namespace"],
                        ...
                        service["flags"],
                        ":%x" %
                        service["ATSC_source_id"] if "ATSC_source_id" in service else ""))

                control_chars = ''.join(
                    list(map(six.unichr, list(range(0, 32)) + list(range(127, 160)))))
                control_char_re = re.compile('[%s]' % re.escape(control_chars))
                if 'provider_name' in service.keys():
                    service_name = control_char_re.sub(
                        '',
                        six.ensure_text(
                            six.ensure_str(
                            ...
                            encoding='utf-8',
                            errors='ignore'))
                    provider_name = control_char_re.sub(
                        '',
                        six.ensure_text(
                            six.ensure_str(
                ...
                service_flags = ""
                if "service_flags" in service.keys(
                ) and service["service_flags"] > 0:
                ...
                if transponder["system"] == 0:  # DVB-S
                    ...
                    if "is_id" in transponder and "pls_code" in transponder and "pls_mode" in transponder:
                        try:  # some images are still not multistream aware after all this time
                            ...
                            if not (transponder["is_id"] == eDVBFrontendParametersSatellite.No_Stream_Id_Filter and transponder["pls_code"] ==
                                    eDVBFrontendParametersSatellite.PLS_Gold and transponder["pls_mode"] == eDVBFrontendParametersSatellite.PLS_Default_Gold_Code):
                ...
                lamedblist.append(
                    ...
                    (service["service_id"],
                     service["namespace"],
                        ...
                        service["flags"],
                        ":%x" %
                        service["ATSC_source_id"] if "ATSC_source_id" in service else ":0"))
    ...
    def __init__(self, session):
        ...
        self.list = [
            getConfigListEntry(
                _("Location"),
                self.tmpfile_path,
                _("Location of the lamedb file to merge into %s/lamedb.") %
                lamedb_path),
        ]
    ...
    def keySave(self):
        ...
        if filename == fileCheck(filename):
            self.session.openWithCallback(
                self.MergeCallback,
                MessageBox,
                _("File found '%s'\nMerge into system lamedb now?") %
                filename,
                MessageBox.TYPE_YESNO)
        else:
            self.session.open(
                MessageBox,
                _("File '%s' could not be found.\nPlease check you have sent it to the correct folder.") %
                filename,
                MessageBox.TYPE_ERROR)

    def MergeCallback(self, answer):
        try:
            backupName = lamedb_path + "/lamedb-" + \
                strftime("%Y-%m-%d_%H-%M-%S", localtime(int(time()))) + ".bak"
            # make a backup just in case
            copy2(lamedb_path + "/lamedb", backupName)
            transponders = LamedbReader().readLamedb(
                path=lamedb_path)  # read /etc/enigma2/lamedb
            transponders = LamedbReader().readLamedb(
                path=self.tmpfile_path.value,
                transponders=transponders)  # read user lamedb
            writer = LamedbWriter()
            writer.writeLamedb(lamedb_path, transponders)
            writer.writeLamedb5(lamedb_path, transponders)
            eDVBDB.getInstance().reloadServicelist()
            msg = _("File successfully merged into '" + lamedb_path +
                    "/lamedb'\nA backup of your previous lamedb has been stored as\n%s") % backupName
        except BaseException:
            import traceback
            traceback.print_exc()
            msg = _(
                "Supplied lamedb could not be merged.\nCheck debug log for further details.")
        self.session.openWithCallback(
            self.close, MessageBox, msg, MessageBox.TYPE_INFO)
