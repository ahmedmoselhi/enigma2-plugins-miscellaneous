...
def PartitionEntry(description, size):
    picture = LoadPixmap(
        ...
        path=resolveFilename(
            ...
            "SystemPlugins/DeviceManager/icons/partitionmanager.png"))
...
class HddPartitions(Screen):
    ...
    def chkfs(self):
        ...
        if ret == 0:
            ...
            self.session.open(
                MessageBox,
                _("Error checking disk. The disk may be damaged"),
                MessageBox.TYPE_ERROR)
    ...
    def mkfs(self):
        ...
        if ret == 0:
            self.session.open(
                MessageBox,
                _("Format terminated with success"),
                MessageBox.TYPE_INFO)
        elif ret == -2:
            self.session.open(
                MessageBox,
    ...
    def domkfs(self, result):
        if self.disk[5][self.index][3] == "83":
            if self.isExt4Supported():
                if result < 3:
                    self.fstype = result
                    self.session.open(ExtraActionBox, _("Formatting disk %s") %
                                      self.disk[5][self.index][0], _("Formatting disk"), self.mkfs)
            else:
                if result < 2:
            ...
            if self.disk[5][self.index][3] == "83":
                if self.isExt4Supported():
                    self.session.openWithCallback(
                        self.domkfs, ExtraMessageBox, _("Format as"), _("Partitioner"), [
                            [
                ...
                self.session.openWithCallback(self.domkfs, ExtraMessageBox, _("Format as"), _(
                    "Partitioner"), [["Fat32", "partitionmanager.png"], [_("Cancel"), "cancel.png"], ], 1, 1)
    ...
    def refreshMP(self, uirefresh=True):
        ...
        for part in self.disk[5]:
            ...
            if (((float(size) / 1024) / 1024) / 1024) > 1:
                capacity = "%d %s" % (
                    int(round((((float(size) / 1024) / 1024) / 1024), 2)), "TB")
            elif ((size / 1024) / 1024) > 1:
                capacity = "%d %s" % (
                    int(round(((float(size) / 1024) / 1024), 2)), "GB")
            else:
                capacity = "%d %s" % (
                    int(round((float(size) / 1024), 2)), "MB")
            ...
            if len(mp) > 0:
                self.partitions.append(
                    PartitionEntry(
                        _("P. %d - %s (Fixed: %s)") %
                        (count, part[2], mp), capacity))
            elif len(rmp) > 0:
                self.partitions.append(
                    PartitionEntry(
                        _("P. %d - %s (Fast: %s)") %
                        (count, part[2], rmp), capacity))
            else:
                self.partitions.append(
                    PartitionEntry(
                        "P. %d - %s" %
                        (count, part[2]), capacity))
            count += 1
    ...
    def blue(self):
        if len(self.disk[5]) > 0:
            ...
            if self.disk[5][index][3] != "83" and self.disk[5][index][
                    3] != "7" and self.disk[5][index][3] != "b" and self.disk[5][index][3] != "c":
                return
        ...
        if len(self.partitions) > 0:
            ...
            if self.mounted:
                ...
                self.session.openWithCallback(
                    self.refreshMP, HddMountDevice, self.disk1, self.sindex + 1)
