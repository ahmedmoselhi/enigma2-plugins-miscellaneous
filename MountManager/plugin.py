...
class DevicesMountPanel(Screen, ConfigListScreen):
	...
	def buildMy_rec(self, device, moremount=[]):
		...
		if not os.path.exists("/usr/sbin/sfdisk"):
			...
			sleep(0.5) # Wait for the opkg command to likely finish (crude, but keeping the original structure)
		...
		try:
			with open('/proc/partitions', 'r') as f:
				...
				if not found_partition:
					# Try to get size from /sys/block/.../size for a whole device
		...
		if size > 0:
			...
			if size_in_bytes >= 1024**4: # TB (if size in KB is used, this is correct)
				des = "%s%d %s" % (_("Size: "), int(round(size_in_bytes / 1024**4, 2)), _("TB"))
			elif size_in_bytes >= 1024**3: # GB
				des = "%s%d %s" % (_("Size: "), int(round(size_in_bytes / 1024**3, 2)), _("GB"))
			elif size_in_bytes >= 1024**2: # MB
				des = "%s%d %s" % (_("Size: "), int(round(size_in_bytes / 1024**2, 2)), _("MB"))
			else: # KB (Unlikely for devices, but complete)
				des = "%s%d %s" % (_("Size: "), int(round(size_in_bytes / 1024, 2)), _("KB"))
		...
		if des != '':
			prev_des = des
			des += '\t' + _("Mount: ") + d1 + '\n' + _("Device: ") + '/dev/' + device + '\t' + _("Type: ") + dtype + rw_status
	...
	def send_reset(self, dev_path=''):
		if dev_path != '':
			...
			try:
				...
				self.session.open(MessageBox, _("Error opening output file '%s'!") % dev_path + "\nError: " + str(e), MessageBox.TYPE_ERROR, timeout=5)
				return
			...
			try:
				...
				try:
					...
					pass
				self.session.open(MessageBox, _("Error in ioctl '%s'!") % dev_path + "\nError: " + str(e), MessageBox.TYPE_ERROR, timeout=5)
	...
	def MountCur3(self):
		...
		if sel:
			...
			des = des.replace('\n', '\t')
			parts = des.strip().split('\t')
			# parts[2] is Device: /dev/device, so parts[2].split() is ['Device:', '/dev/device']
			self.device = parts[2].replace(_("Device: "), '').replace('/dev/', '')
			...
			self.Console = Console()
			self.Console.ePopen("/sbin/blkid | grep /dev/" + self.device, self.cur_in_fstab, [self.device, self.mountp])
	...
	def UnmountAnswer(self, answer):
		if answer:
			...
			if sel:
				...
				mountp = parts[1].replace(_("Mount: "), '')
				device = parts[2].replace(_("Device: "), '') # /dev/device
				moremount = sel[3] # This is either "" or an extended mount message

				# Unmount by mount point first
				if mountp != _("None"):
					system('umount -f ' + mountp) # use -f for forceful unmount
				
				# If no extended mounts (i.e. only one mount point shown), try unmounting by device
				if moremount == "":
	...
	def saveMypoints(self):
		...
		if sel:
			...
			# Red button is "Use as HDD". Check if it's already mounted as /media/hdd
	...
	def saveMypointAnswer(self, answer):
		if answer:
			...
			if sel:
				...
				des = des.replace('\n', '\t')
				parts = des.strip().split('\t')
				self.mountp = parts[1].replace(_("Mount: "), '') # Current mountpoint (could be "None")
				self.device = parts[2].replace(_("Device: "), '') # /dev/device
				device_only = self.device.replace('/dev/', '') # device (e.g., sda1)
				
				# Stop swap/transmission and unmount /media/hdd first
				system('[ -e /media/hdd/swapfile ] && swapoff /media/hdd/swapfile')
				# system('[ -e /etc/init.d/transmissiond ] && /etc/init.d/transmissiond stop') # Commented out in original
				...
				if self.mountp.find('/media/hdd') < 0:
					...
					self.Console.ePopen("/sbin/blkid | grep " + self.device, self.add_fstab, [device_only, self.mountp])
	...
	def add_fstab(self, result=None, retval=None, extra_args=None):
		...
		if not self.device_uuid:
			print(f"[MountManager] error get UUID for device {self.device_only}")
			return
		...
		if fileExists("/usr/lib/enigma2/python/Plugins/Extensions/Flashexpander/plugin.pyo") or fileExists("/usr/lib/enigma2/python/Plugins/Extensions/Flashexpander/plugin.pyc"):
			try:
				with open("/etc/fstab", 'r') as f:
					for line in f.readlines():
						if '/usr' in line and line.startswith(self.device_uuid):
							flashexpander = line
			except:
				pass
		
		# Clean up old entries related to /media/hdd, the old device name, or the old UUID
		try:
			...
			with open('/etc/fstab', 'r') as f_in:
				lines = [l for l in f_in.readlines() if self.device_only not in l]
			with open('/etc/fstab.tmp', 'w') as f_out:
				f_out.writelines(lines)
			os.rename('/etc/fstab.tmp', '/etc/fstab')
			...
			with open('/etc/fstab', 'r') as f_in:
				lines = [l for l in f_in.readlines() if self.device_uuid not in l]
			with open('/etc/fstab.tmp', 'w') as f_out:
				f_out.writelines(lines)
			os.rename('/etc/fstab.tmp', '/etc/fstab')
		except:
...
class DeviceMountPanelConf(Screen, ConfigListScreen):
	...
	def buildMy_rec(self, device):
		...
		if 'MMC' in name:
			choices.extend([('/media/mmc', '/media/mmc'), ('/media/mmc1', '/media/mmc1'), ('/media/mmc2', '/media/mmc2')])
		
		# Check for Flashexpander for the /usr option
		if not fileExists("/usr/lib/enigma2/python/Plugins/Extensions/Flashexpander/plugin.pyo") and not fileExists("/usr/lib/enigma2/python/Plugins/Extensions/Flashexpander/plugin.pyc"):
	...
	def renameEntryCallback(self, answer):
		if answer is not None:
			...
			if ret == 0:
				...
				self.label_device = ""
				self.session.open(MessageBox, _("Failed to set label!"), MessageBox.TYPE_ERROR)
				self.close()
	...
	def delCurrentUUID(self, result=None, retval=None, extra_args=None):
		...
		try:
			# Remove entry by device name
			with open('/etc/fstab', 'r') as f_in:
				lines = [l for l in f_in.readlines() if device_name_only not in l]
			with open('/etc/fstab.tmp', 'w') as f_out:
				f_out.writelines(lines)
			os.rename('/etc/fstab.tmp', '/etc/fstab')
	...
	def saveMypointsAnswer(self, answer):
		if answer:
			...
			for x in self['config'].list:
				...
				# Get blkid info (UUID/TYPE)
				self.Console.ePopen("/sbin/blkid | grep /dev/" + self.device, self.add_fstab, [self.device, self.mountp])
		...
		if fileExists(file_mounts):
			...
			for line in lines_mount:
				if '/omb' not in line:
					...
					if len(l) > 0 and (l[0].startswith('/dev/sd') or l[0].startswith('/dev/mmcblk')):
	...
	def add_fstab(self, result=None, retval=None, extra_args=None):
		...
		if result and " UUID=" in result:
			...
			for item in self.device_tmp:
				if item.startswith('UUID='):
					...
					self.device_type = item.replace('TYPE=', "").replace('"', "").replace('\n', "")
					
		if not self.device_type or not self.device_uuid:
			print(f"[MountManager] error reading UUID/TYPE device {self.device}")
			return
		...
		# Clean up old entries related to the new mount point, the device name, or the UUID
		try:
		...
		try:
			with open('/etc/fstab', 'a') as out:
				line = f"{self.device_uuid}\t{self.mountp}\t{self.device_type}\tdefaults\t0 2\n"
				if flashexpander is not None:
					line += flashexpander
				out.write(line)
		except Exception as e:
