...
def supportExtFat():
    if not os.path.isfile("/sbin/mkexfatfs"):
        ...
        if 'mips' in arch and os.path.isfile(
                "/usr/lib/enigma2/python/Plugins/SystemPlugins/DeviceManager/bin/mips/mkexfatfs"):
            ...
            os.system(
                "cp /usr/lib/enigma2/python/Plugins/SystemPlugins/DeviceManager/bin/mips/exfatfsck /sbin/exfatfsck && chmod 755 /sbin/exfatfsck")
            ...
            os.system(
                "cp /usr/lib/enigma2/python/Plugins/SystemPlugins/DeviceManager/bin/armv7l/exfatfsck /sbin/exfatfsck && chmod 755 /sbin/exfatfsck")
    if "exfat-fuse" in open("/etc/filesystems").read():
        pass
    else:
        os.system(
            "echo exfat-fuse >> /etc/filesystems && opkg update && opkg install fuse-exfat")
...
def Plugins(**kwargs):
    return [
        PluginDescriptor(
            ...
            description=_("Format/Partition your Devices and manage Mountpoints"),
            where=PluginDescriptor.WHERE_MENU,
            fnc=deviceManagerSetup),
        PluginDescriptor(
            name=_("Device Manager - Fast Mounted Remove"),
            description=_("Quick and safe remove for your mounted devices "),
            where=PluginDescriptor.WHERE_EXTENSIONSMENU,
            fnc=deviceManagerFastRemove)]
