    ...
    if not removable and internal and rotational:
        picture = LoadPixmap(
            ...
            path=resolveFilename(
                ...
                "SystemPlugins/DeviceManager/icons/disk.png"))
    elif internal and not rotational:
        picture = LoadPixmap(
            cached=True,
            path=resolveFilename(
                SCOPE_CURRENT_PLUGIN,
                "SystemPlugins/DeviceManager/icons/ssddisk.png"))
    else:
        picture = LoadPixmap(
            cached=True,
            path=resolveFilename(
                SCOPE_CURRENT_PLUGIN,
                "SystemPlugins/DeviceManager/icons/diskusb.png"))
...
class HddSetup(Screen):
    ...
    def format(self, result):
        if result != 0:
            self.session.open(
                MessageBox, _("Cannot format partition %d") %
                (self.formatted), MessageBox.TYPE_ERROR)
        if self.result == 0:
            if self.formatted > 0:
                self.checkDefault()
                self.refresh()
        ...
        self.session.openWithCallback(
            self.format, ExtraActionBox, _("Formatting partition %d") %
            (self.formatted + 1), _("Initialize disk"), self.mkfs)
    ...
    def fdisk(self):
        ...
        # if "mmcblk" in disk1:
        # disk1 = disk1 + "p"
        return self.mdisks.fdisk(disk1,
    ...
    def chooseFSType(self, result):
        if result != 5:
            ...
            if self.isExt4Supported():
                self.session.openWithCallback(
                    self.initialaze, ExtraMessageBox, _("Format as"), _("Partitioner"), [
                        [
    ...
    def yellowAswer(self):
        if sfdisk and len(self.mdisks.disks) > 0:
            ...
            self.session.openWithCallback(self.chooseFSType,
                                          ExtraMessageBox,
                                          ...
                                          [[_("One partition"),
                                            "partitionmanager.png"],
                                              [_("Two partitions (50% - 50%)"),
                                               "partitionmanager.png"],
                                              [_("Two partitions (75% - 25%)"),
                                               "partitionmanager.png"],
    ...
    def green(self):
        if len(self.mdisks.disks) > 0:
            ...
            self.session.open(HddInfo,
                              self.mdisks.disks[self.sindex][0],
    ...
    def blue(self):
        if len(self.mdisks.disks) > 0:
            ...
            if len(self.mdisks.disks[self.sindex][5]) == 0:
                ...
                self.session.open(
                    HddPartitions, self.mdisks.disks[self.sindex])
