...
# Global variables for fstab entries and max lengths (maintained for Enigma2 structure)
entryList = []
lengthList = [0, 0, 0, 0]
...
class fstabViewerScreen(Screen, HelpableScreen):
	...
	def openEditScreen(self):
		if not self.builderror:
			...
			# Check if the currently selected entry is a system entry before opening a confirmation dialog
	...
	def buildScreen(self):
					...
					for line in fstabFile:
						...
						if line.strip() and not line.startswith("#"):
							try:
								...
								entry = entry[:6] # Take only the first 6 fields
								entryList.append(entry)

								# Calculate max lengths for pretty writing back to file
								...
								# If parsing an individual entry fails, set error state and stop
								self.fstabEntryList = []
		...
		self["menulist"].l.setList(self.fstabEntryList)
		self["entryinfo"].setText(f"{self['menulist'].getSelectedIndex() + 1} / {self.counter}") # Use f-string (style improvement)
			...
			try:
				...
				with open('/etc/fstab', 'w') as configFile:
					for i in range(len(entryList)):
						# Keep old % formatting here as it's cleaner for fixed-width alignment
						line = "%*s %*s %*s %*s %s %s\n" % (
	...
	def mountall(self):
		if not self.builderror:
			# Use subprocess.run or alternative if possible, but os.system is common in Enigma2
			os.system("mount -a")

	def checkSoftwareEntry(self, entry=''):
		# Check for common system-related entries
		system_entries = ["proc", "tmpfs", "jffs2", "usbfs", "usbdevfs", "devpts", "rootfs"]
	...
	def addEntry(self):
		if not self.builderror:
			...
			# Only allow cloning if an entry is selected AND it's not a system entry
			if selectedEntry is not None and selectedEntry >= 0 and not self.checkSoftwareEntry(entryList[selectedEntry][0]):
		...
		if len(list_actions) > 0:
			if len(list_actions) == 1:
				...
				def setAction(choice):
					if choice is not None:
						self._execute_restore(choice[1])
				self.session.openWithCallback(setAction, ChoiceBox, title=_("Select action:"), list=list_actions)
		else:
	...
	def _execute_restore(self, action_type):
		...
		if source_file:
			...
			try:
				...
				self.session.open(MessageBox, _(f"Failed to restore fstab: {e}"), MessageBox.TYPE_ERROR, timeout=8) # Use f-string (style improvement)
		else:
			self.session.open(MessageBox, _("Error during restore operation!"), MessageBox.TYPE_ERROR, timeout=5)
...
class fstabEditorScreen(Screen, ConfigListScreen, HelpableScreen):
	...
	def __init__(self, session, selectedEntry=None, addEntry=False, cloneEntry=False):
		...
		if self.addEntry:
			...
			# Ensure the default value is one of the available options for fstype
			initial_fstype = entry[2] if entry[2] in [t[0] for t in fstypes] else "auto"

			self.devicename = NoSave(ConfigText(fixed_size=False, default=entry[0]))
			self.mountpoint = NoSave(ConfigText(fixed_size=False, default=entry[1]))
			self.fstype = NoSave(ConfigSelection(fstypes, default=initial_fstype))
			self.options = NoSave(ConfigText(fixed_size=False, default=entry[3]))
			...
			self.dumpfreq = NoSave(ConfigInteger(limits=(0, 1), default=initial_dumpfreq))
			
			# Ensure passnum is one of the valid options
			initial_passnum = entry[5] if entry[5] in ["0", "1", "2"] else "0"
			self.passnum = NoSave(ConfigSelection([("0", "0"), ("1", "1"), ("2", "2")], default=initial_passnum))
	...
	def selectionChanged(self):
		...
		# Show OK button only for device name or mount point, as they use the file selector
		current_entry = self.getCurrentEntry()
		if current_entry == _("device name: ") or current_entry == _("mount point: "):
			self["ButtonOK"].show()
		else:
			self["ButtonOK"].hide()
	...
	def checkEntry(self):
		...
		if not self.devicename.value.strip() or not self.mountpoint.value.strip() or not self.options.value.strip():
	...
	def saveEntry(self):
		...
		if new_entry in entryList:
			if not (self.addEntry or self.cloneEntry) and entryList[self.selectedEntry] == new_entry:
				# It's the same entry being edited, so it's not a true duplicate
				pass
			else:
		...
		try:
			...
			# Ignore errors during directory creation (e.g., read-only filesystem)
			pass
