...
# Global variables for fstab entries and max lengths (maintained for
# Enigma2 structure)
entryList = []
lengthList = [0, 0, 0, 0]
...
class fstabMenuList(MenuList):
    def __init__(self, list):
        ...
        # skin.xml <alias name="fstabMenuList" font="Regular" size="28"
        # height="50"/>
        try:
...
def fstabMenuListEntry(
    ...
    # skin.xml <parameter name="fstabMenuList"
    # value="0,30,600,50,700,30,500,50" />
    try:
        x, y, w, h, x1, y1, w1, h1 = skin.parameters.get(
            "fstabMenuList", (0, 0, 200, 25, 230, 0, 370, 25))
    except BaseException:
        x = 0
        y = 0
        w = 200
    ...
    # Using MultiContentEntryText to display aligned text
    res.append(
...
class fstabViewerScreen(Screen, HelpableScreen):
    # Directly assign skin here to follow standard Enigma2 skinning practice
    skin = """
    ...
    def __init__(self, session, args=0):
        ...
        self["ColorActions"] = HelpableActionMap(
            self, "ColorActions", {
                "green": (
                    self.addEntry, _("Add entry")), "yellow": (
                    self.restoreBackUp, _("Restore backup fstab")), "blue": (
                    self.mountall, _("Run mount -a")), "red": (
                        self.close, _("Close plugin")), }, -1)

        self["OkCancelActions"] = HelpableActionMap(
            self, "OkCancelActions", {
                "cancel": (
                    self.close, _("Close plugin")), "ok": (
                    self.openEditScreen, _("Open editor")), }, -1)
    ...
    def openEditScreen(self):
        if not self.builderror:
            ...
            # Check if the currently selected entry is a system entry before
            ...
            if not self.checkSoftwareEntry(entryList[self.selectedEntry][0]):
                ...
                self.session.openWithCallback(
                    self.openEditSystemEntryScreen,
                    MessageBox,
                    _("Attention!\nNot highly recommend that you remove or edit the system entry!\n") +
                    _("Really edit the selected entry?"),
    ...
    def buildScreen(self):
        ...
        if fileExists("/etc/fstab"):
            try:
                # Use 'with open' for safe file handling (logic improvement)
                with open("/etc/fstab", "r") as fstabFile:
                    for line in fstabFile:
                        ...
                        if line.strip() and not line.startswith("#"):
                            try:
                                entry = line.split()
                                # fstab expects exactly 6 fields, though split() might return more if there are extra spaces
                                # We only care about the first 6
                                if len(entry) < 6:
                                ...
                                # Take only the first 6 fields
                                entry = entry[:6]
                                entryList.append(entry)

                                # Calculate max lengths for pretty writing back
                                # to file
                                ...
                                # If parsing an individual entry fails, set
                                # error state and stop
                                self.fstabEntryList = []
                                self["menulist"].l.setList(self.fstabEntryList)
                                self["entryinfo"].setText(
                                    _('Failed to read /etc/fstab! Restore fstab.backup or edit the file manually.'))
                                self.builderror = True
                                return -1
            except Exception:
                # If file reading fails (e.g., permissions), set error state
                self.fstabEntryList = []
                self["menulist"].l.setList(self.fstabEntryList)
                self["entryinfo"].setText(
                    _('Failed to read /etc/fstab! Restore fstab.backup or edit the file manually.'))
        ...
        self["menulist"].l.setList(self.fstabEntryList)
        # Use f-string (style improvement)
        self["entryinfo"].setText(
            f"{self['menulist'].getSelectedIndex() + 1} / {self.counter}")

    def writeFile(self, returnvalue):
        if returnvalue != 0 and not self.builderror:
            # Create backup before writing new file
            os.system("cp /etc/fstab /etc/fstab.backup")

            try:
                # Use 'with open' for safe file handling (logic improvement)
                with open('/etc/fstab', 'w') as configFile:
                    for i in range(len(entryList)):
                        # Keep old % formatting here as it's cleaner for
                        # fixed-width alignment
                        line = "%*s %*s %*s %*s %s %s\n" % (
                            int(lengthList[0]) * -1, entryList[i][0],
                            int(lengthList[1]) * -1, entryList[i][1],
                            int(lengthList[2]) * -1, entryList[i][2],
                            int(lengthList[3]) * -1, entryList[i][3],
                            str(entryList[i][4]), str(entryList[i][5]))
    ...
    def mountall(self):
        if not self.builderror:
            # Use subprocess.run or alternative if possible, but os.system is
            # common in Enigma2
            os.system("mount -a")

    def checkSoftwareEntry(self, entry=''):
        # Check for common system-related entries
        system_entries = [
            "proc",
            "tmpfs",
            "jffs2",
            ...
            "devpts",
            "rootfs"]
        if entry and any(sys_entry in entry for sys_entry in system_entries):
            return True
        return False
    ...
    def addEntry(self):
        if not self.builderror:
            ...
            # Only allow cloning if an entry is selected AND it's not a system
            # entry
            if selectedEntry is not None and selectedEntry >= 0 and not self.checkSoftwareEntry(
                    entryList[selectedEntry][0]):
                ...
                def setAction(choice):
                    if choice is not None:
                        if choice[1] == "new":
                            ...
                            self.cloneEntry()
                self.session.openWithCallback(
                    setAction, ChoiceBox, title=_("Select action:"), list=menu)
    ...
    def restoreBackUp(self):
        ...
        if backup:
            list_actions.append((_("Restore fstab.backup"), "backup"))
        default = fileExists("/etc/fstab-opkg")
        if default:
            list_actions.append((_("Restore default fstab"), "default"))

        if len(list_actions) > 0:
            if len(list_actions) == 1:
                # If only one option, execute it directly
                choice = list_actions[0]
                self._execute_restore(choice[1])
            else:
                # Open ChoiceBox for multiple options
                def setAction(choice):
                    if choice is not None:
                        self._execute_restore(choice[1])
                self.session.openWithCallback(
                    setAction, ChoiceBox, title=_("Select action:"), list=list_actions)
        else:
            self.session.open(
                MessageBox,
                _("Not found restore file!"),
                MessageBox.TYPE_ERROR,
                timeout=5)

    def _execute_restore(self, action_type):
        """Internal method to handle the actual file restoration."""
        source_file = ""
        if action_type == "backup":
            source_file = "/etc/fstab.backup"
        elif action_type == "default":
            source_file = "/etc/fstab-opkg"

        if source_file:
            try:
                # Atomic-like restore: delete old fstab and copy backup/default
                os.remove("/etc/fstab")  # Use os.remove instead of rm -f
            except OSError:
                pass  # Ignore if file doesn't exist

            try:
                # Use standard file copy to restore (safer than os.system)
                with open(source_file, 'r') as src, open("/etc/fstab", 'w') as dst:
                    dst.write(src.read())
                ...
                self.session.open(MessageBox,
                                  _(f"Failed to restore fstab: {e}"),
                                  MessageBox.TYPE_ERROR,
                                  timeout=8)  # Use f-string (style improvement)
        else:
            self.session.open(
                MessageBox,
                _("Error during restore operation!"),
                MessageBox.TYPE_ERROR,
                timeout=5)
...
class fstabEditorScreen(Screen, ConfigListScreen, HelpableScreen):
    ...
    def __init__(
        ...
        self["ColorActions"] = HelpableActionMap(
            self, "ColorActions", {
                "green": (
                    self.checkEntry, _("Return with saving")), "red": (
                    self.cancelEntry, _("Return without saving")), "blue": (
                    self.removeEntry, _("Remove entry")), }, -1)

        self["OkCancelActions"] = HelpableActionMap(
            self, "OkCancelActions", {
                "cancel": (
                    self.cancelEntry, _("Return without saving")), "ok": (
                    self.ok, _("Open selector")), }, -1)
        ...
        # Configuration setup
        # Define all possible fstypes
        fstypes = [
            ("auto", "auto"), ("ext2", "ext2"), ("ext3", "ext3"), ("ext4", "ext4"),
            ("cifs", "cifs"), ("nfs", "nfs"), ("swap", "swap"), ("btrfs", "btrfs"),
            ("ntfs-3g", "ntfs-3g"), ("vfat", "vfat"), ("fat", "fat"), ("ntfs", "ntfs"),
            ("udf", "udf"), ("iso9660", "iso9660"), ("cdfs", "cdfs"), ("exfat", "exfat")
        ]
        ...
        if self.addEntry:
            ...
            self.fstype = NoSave(ConfigSelection(fstypes, default="auto"))
            self.options = NoSave(
                ConfigText(
                    default="defaults",
                    fixed_size=False))
            self.dumpfreq = NoSave(ConfigInteger(limits=(0, 1), default=0))
            self.passnum = NoSave(ConfigSelection(
                [("0", "0"), ("1", "1"), ("2", "2")], default="0"))
        else:
            # Values based on selected/cloned entry
            entry = entryList[self.selectedEntry]

            # Ensure the default value is one of the available options for
            # fstype
            initial_fstype = entry[2] if entry[2] in [t[0]
                                                      for t in fstypes] else "auto"
            ...
            self.fstype = NoSave(
                ConfigSelection(
                    fstypes,
                    default=initial_fstype))
            self.options = NoSave(
                ConfigText(
                    fixed_size=False,
                    default=entry[3]))

            # Ensure dumpfreq is an integer within limits (0 or 1)
            initial_dumpfreq = 0
            try:
                initial_dumpfreq = max(0, min(1, int(entry[4])))
            except ValueError:
            ...
            self.dumpfreq = NoSave(
                ConfigInteger(
                    limits=(
                        0,
                        1),
                    default=initial_dumpfreq))

            # Ensure passnum is one of the valid options
            initial_passnum = entry[5] if entry[5] in ["0", "1", "2"] else "0"
            self.passnum = NoSave(ConfigSelection(
                [("0", "0"), ("1", "1"), ("2", "2")], default=initial_passnum))
        ...
        self.list.append(
            getConfigListEntry(
                _("file system type (auto): "),
                self.fstype))
        self.list.append(
            getConfigListEntry(
                _("options mount file system (defaults): "),
                self.options))
        self.list.append(
            getConfigListEntry(
                _("dump (0 - create backup off): "),
                self.dumpfreq))
        self.list.append(
            getConfigListEntry(
                _("pass num (0 - fsck off): "),
                self.passnum))

        self["config"].setList(self.list)
        self["config"].onSelectionChanged.append(self.selectionChanged)
        self.selectionChanged()  # Initial call to set OK button state
    ...
    def selectionChanged(self):
        ...
        # Show OK button only for device name or mount point, as they use the
        # file selector
        current_entry = self.getCurrentEntry()
        if current_entry == _(
                "device name: ") or current_entry == _("mount point: "):
            self["ButtonOK"].show()
        else:
            self["ButtonOK"].hide()

    def checkEntry(self):
        # Simplified mandatory field check
        if not self.devicename.value.strip() or not self.mountpoint.value.strip(
        ) or not self.options.value.strip():
            self.session.open(
                MessageBox,
                _("Please enter a value for every input field!"),
                MessageBox.TYPE_ERROR,
                timeout=6)
        else:
            self.saveEntry()
    ...
    def saveEntry(self):
        # Redundant mandatory field check, but kept for safety after checkEntry
        if not self.devicename.value.strip() or not self.mountpoint.value.strip(
        ) or not self.options.value.strip():
            self.session.open(
                MessageBox,
        ...
        # Logic check: pass num 1 is typically only for root filesystem
        if self.passnum.value == "1" and self.mountpoint.value != "/":
            self.session.open(
                MessageBox,
                _("Pass num value 1 use only for root filesystem (mountpoint '/')!"),
                MessageBox.TYPE_ERROR,
                timeout=6)
            return
        ...
        if new_entry in entryList:
            if not (
                    self.addEntry or self.cloneEntry) and entryList[self.selectedEntry] == new_entry:
                # It's the same entry being edited, so it's not a true
                # duplicate
                pass
            else:
                self.session.open(
                    MessageBox,
                    _("This entry already exists in the fstab!"),
                    MessageBox.TYPE_ERROR,
                    timeout=6)
                return
        ...
        # Apply changes to the global entryList
        if self.addEntry or self.cloneEntry:
        ...
        try:
            if not os.path.exists(self.mountpoint.value):
                # Use a more secure and Pythonic way to create directory
                os.makedirs(self.mountpoint.value, mode=0o755, exist_ok=True)
        except Exception:
            # Ignore errors during directory creation (e.g., read-only
            # filesystem)
            pass

        # Close with return value 1 to trigger writeFile in fstabViewerScreen
        self.close(1)

    def removeEntry(self):
        if not self.addEntry and not self.cloneEntry:
            self.session.openWithCallback(
                self.removeEntryAnswer,
                MessageBox,
                self.checkSoftwareEntry() +
                _("Really delete the selected entry?"),
                MessageBox.TYPE_YESNO)
    ...
    def cancelEntry(self):
        # Close with return value 0 to indicate no save needed
        self.close(0)

    def ok(self):
        if self.cloneEntry:
            return  # OK button is hidden in clone mode

        current_index = self["config"].getCurrentIndex()
        if current_index is None:
            return

        # Select dir/file based on selected config item
        current_entry_name = self.list[current_index][0]

        if current_entry_name == _("mount point: "):
            # Start in /media/ to select a mount point (directory)
            self.session.openWithCallback(
                self.dirSelectDlgClosed, dirSelectDlg, "/media/dummy/", False)
        elif current_entry_name == _("device name: "):
            # Start in /dev/ to select a device name (file/device)
            self.session.openWithCallback(
                self.dirSelectDlgClosed, dirSelectDlg, "/dev/dummy/", True)

    def dirSelectDlgClosed(self, selection):
        # dirSelectDlg returns the path string or False on cancel
        if selection is not False:
            ...
            if current_entry_name == _("mount point: "):
                self.mountpoint.value = selection
            elif current_entry_name == _("device name: "):
                self.devicename.value = selection
